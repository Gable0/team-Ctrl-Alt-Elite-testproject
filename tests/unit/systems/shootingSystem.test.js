...
test('triple shot fires 3 bullets', () => {
    ...
    if (game.tripleShotTimer > 0) {
        game.playerShots.push(createPlayerShot(game.player.x, game.player.y - game.player.size, -0.2))
        game.playerShots.push(createPlayerShot(game.player.x, game.player.y - game.player.size, 0))
        game.playerShots.push(createPlayerShot(game.player.x, game.player.y - game.player.size, 0.2))
    }
...
test('normal shot fires 1 bullet', () => {
    ...
    if (game.tripleShotTimer > 0) {
        ...
        game.playerShots.push(createPlayerShot(game.player.x, game.player.y - game.player.size))
    }
...
test('player shots deactivate when off top', () => {
    const game = {
        playerShots: [
            { x: 400, y: -100, vx: 0, vy: -800, active: true, update: function(delta) {
