...
test('player shot kills enemy and removes bullet (no piercing)', () => {
    ...
    let killed = false
    checkPlayerShotCollisions(game, () => { killed = true })
...
test('player shot ignores dying enemies', () => {
    ...
    checkPlayerShotCollisions(game, () => { assert.fail('should not call') })
...
test('multiple player shots — only one hits per bullet', () => {
    ...
    let killCount = 0
    checkPlayerShotCollisions(game, () => { killCount++ })
...
test('enemy shot hits player — returns true and removes shot', () => {
    ...
    let hit = false
    const result = checkEnemyShotCollisions(game, () => { hit = true })
...
test('enemy shot misses player — returns false', () => {
    ...
    const result = checkEnemyShotCollisions(game, () => { assert.fail('should not hit') })
